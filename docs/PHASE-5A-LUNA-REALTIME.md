...
2. **Gateway:** Continue single path `ingestOpenAiAudioDelta` → A2F `ingestChunk` + realtime adapter toward EchoMimic (8889). Prefer **multiplex** `{ pcm16, a2fEnvelope, ts }` from gateway → 8899 unless RunPod internal A2F→Echo wiring is chosen later.
...
- [ ] **Gateway:** Feed aligned A2F envelopes + PCM into 8899; handle reconnect.
